...
The SDK embeds the same inventories generated by `incident-py-q`. Golden methods
are exposed directly on `client.<Namespace>.<Method>`. Silver fallback routes are
exposed under `client.Silver.<Namespace>.<Method>`.
...
This Go SDK is usable for generated Golden wrappers, generated Silver fallback
